...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            #print (data)
            #dataset1=pd.read_csv("new_heart.csv",index_col=None)
            dicc={'yes':1,'no':0}
            filename = 'random_forest_heart_model.pkl.sav'
            classifier = pickle.load(open(filename, 'rb'))

            data = np.array([data_age,data_cp,data_thalach,data_exang,data_oldpeak,data_ca])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_age':data_age,'data_cp':data_cp,'data_thalach':data_thalach,'data_exang':data_exang,'data_oldpeak':data_oldpeak,
                                                        'data_ca':data_ca,'out':out})
